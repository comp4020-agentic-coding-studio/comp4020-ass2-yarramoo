(* Checkpoint 2: success/failure control flow, DEFINE/RETURN/FRETURN.
   ...
   Week 10 widens [value] with two new constructors, [VArray] and
   [VTable] (research/snobol/02-language-reference.md, "Arrays and
   tables"): `ARRAY(n)` / `ARRAY(n, init)` and `TABLE()` / `TABLE(n)`.
   Both parse as ordinary [Call] nodes -- like the pattern primitives
   above, they are told apart from a real DEFINE'd call purely by name
   ([is_data_constructor]/[eval_data_constructor], following the exact
   same dispatch shape as [is_comparison]/[eval_compare]). `A<I>` reads
   an element (ast.ml's [Index]); `A<I> = expr` writes one
   ([IndexAssign], in [exec_stmt]). Both a bad index (out of bounds on
   an array, or a missing key on a table) are genuine SNOBOL4-level
   failures -- [Fail_signal], not an OCaml exception -- exactly like a
   failed pattern match or a division by zero above; using a
   non-array/non-table value as if it were one, by contrast, is a
   host-language type error and is allowed to [failwith] and crash the
   interpreter, the same treatment [to_pattern]/[to_int] already give a
   [VPattern] used where a plain value was expected. [VArray]/[VTable]
   need no extra indirection for SNOBOL4's table-identity semantics
   (mutating a value retrieved from a table is visible on a second
   retrieval by the same key): an OCaml array and a [Hashtbl] are
   already mutable in place and are never copied on lookup.

   This file's dune stanza also turns on `-warn-error +8`
   (non-exhaustive match) starting week 10, so widening [value] is
   deliberately used as the mechanism that forces every existing
   [match] over a [value] (here, in [string_of_value]/[to_int]/
   [to_pattern]) to be revisited and given an explicit new case, rather
   than silently falling through.

   Week 11 adds source positions to the AST (ast.ml), but deliberately
   does not touch evaluation semantics at all -- every function below
   still pattern-matches on the same [expr_kind]/[stmt_body] shapes as
   week 10, just reached through [e.ek] instead of bare [e] wherever a
   function used to match directly on an [expr] (see ast.ml's module
   comment for why [expr] is now a wrapper record). No [pos] value is
   ever inspected here: positions exist for parser.ml's diagnostics, not
   for the evaluator, which only ever cares about what a node *means*,
...
let string_of_value = function
  ...
  | VArray _ -> failwith "cannot use an array value as a plain string"
  | VTable _ -> failwith "cannot use a table value as a plain string"
...
let to_int = function
  ...
  | VArray _ -> failwith "cannot use an array value where a number was expected"
  | VTable _ -> failwith "cannot use a table value where a number was expected"
...
let to_pattern = function
  ...
  | VArray _ -> failwith "cannot use an array value as a pattern"
  | VTable _ -> failwith "cannot use a table value as a pattern"
...
(* Data-constructor calls, `ARRAY(...)`/`TABLE(...)` (research/snobol/
   02-language-reference.md, "Arrays and tables") -- told apart from a
   real DEFINE'd call purely by name, exactly like [is_comparison]/
   [eval_compare] and [is_pattern_primitive]/[eval_pattern_primitive]
   above. `ARRAY(n)` defaults every element to the null string, as real
   SNOBOL4 does when no initial value is given; `ARRAY(n, init)` fills
   every element with [init] instead. `TABLE()`/`TABLE(n)` both build an
   empty, dynamically-growing table -- [n] is only ever a starting
   capacity hint here (real SNOBOL4 tables grow unboundedly regardless
...
let eval_data_constructor (name : string) (args : value list) : value =
  ...
  | _ -> failwith (Printf.sprintf "%s: unexpected number of arguments" name)
...
let rec eval_expr (e : expr) : value =
  ...
  | Index (name, idx_e) ->
    (* A bad index/key is a genuine SNOBOL4-level failure ([Fail_signal]),
       not a crash -- see the module comment. Using a non-array/
       non-table value as if it were one, by contrast, is a
       host-language type error and is allowed to [failwith]. *)
    let i = eval_expr idx_e in
    (match lookup name with
     | VArray arr ->
       let n = to_int i in
       if n >= 1 && n <= Array.length arr then arr.(n - 1) else raise Fail_signal
     | VTable tbl ->
       (match Hashtbl.find_opt tbl i with Some v -> v | None -> raise Fail_signal)
     | _ -> failwith (Printf.sprintf "%s is not an array or table" name))
  | Call (name, args) ->
    ...
    else if is_data_constructor name then eval_data_constructor name arg_vals
    else
  ...
  | IndexAssign (name, idx_e, obj_e) ->
    (try
       ...
       (match lookup name with
        | VArray arr ->
          let n = to_int i in
          if n >= 1 && n <= Array.length arr then begin
            arr.(n - 1) <- v;
            Success
          end else Failure
        | VTable tbl ->
