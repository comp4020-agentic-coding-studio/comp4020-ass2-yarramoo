...
(* Pattern primitives (research/snobol/03-pattern-matching.md, "The
   ...
   like [is_comparison]/[eval_compare] above. [ARB] and [BAL] are bare
   constants, not functions, so they are handled separately in
   [eval_expr]'s [Var] case instead of here. *)
let is_pattern_primitive name =
  match name with
  | "LEN" | "ANY" | "NOTANY" | "SPAN" | "BREAK" | "ARBNO" -> true
  | _ -> false

(* TODO(checkpoint 3, v2): implement [eval_pattern_primitive] -- build the
   ...
   SPAN/BREAK are greedy runs, ARBNO is ARB's growth-by-retry
   generalized to a whole subpattern (see [match_pat] below for their
   precise semantics). *)
let eval_pattern_primitive (_name : string) (_args : value list) : value =
  failwith "TODO: implement eval_pattern_primitive (LEN/ANY/NOTANY/SPAN/BREAK/ARBNO -> VPattern ...)"
...
(* TODO(checkpoint 3, v2): implement [match_pat], the CPS backtracking
   ...
   *everything after this point* ultimately succeeds -- return [true]
   ...
   patterns"):
     ...
     - PArbno p    : `ARBNO(p) = NULL | p *ARBNO(p)` -- try the null
                     match first by calling [k] immediately, unchanged;
                     on backtrack, match [p] once and recurse into
                     [PArbno p] again from wherever [p] left the
                     cursor, still aimed at the same [k]. This recursive
                     definition is directly executable in a CPS
                     matcher -- no explicit length loop needed, unlike
                     [PArb] above.
