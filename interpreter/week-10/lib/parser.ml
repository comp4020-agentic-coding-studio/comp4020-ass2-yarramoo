(* Checkpoint 2 extends checkpoint 1's expression parser with function
   ...
   array/table indexing (ast.ml's [Index]). It has to live in
   [parse_primary], not as its own precedence level, because it is a
   read used anywhere an ordinary operand is (`X = A<1> + 1`), exactly
   like a call. The one new statement shape this adds is
   [subject = object] where [subject] is now allowed to be an [Index]
   as well as a plain [Var] -- see [parse_body]'s [IndexAssign] case. *)
